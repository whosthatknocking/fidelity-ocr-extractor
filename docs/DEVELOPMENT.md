...
- Python 3.9+
- local OCR backend:
  - `tesseract` preferred when installed
  - otherwise macOS with `swift` and Vision available
...
- The monitoring extraction contract is loaded from `config.toml`.
- Use `FIDELITY_OCR_ENGINE=vision` or `FIDELITY_OCR_ENGINE=tesseract` to force one backend during debugging.
...
- Image quality gates should fail early on screenshots that are too small, too low-contrast, or cannot be mapped to the full canonical monitoring header set.
